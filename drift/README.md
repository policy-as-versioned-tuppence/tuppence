...
**Pre-registration is measured, not asserted.** `grade` reads the commit that introduced these fact
ids into `window.yaml` — from git, first-parent, on the served ref — and refuses to score either
fact against a sample taken earlier. A branch commit registers nothing. Rewriting a fact after it
lands re-registers it, and every score taken against the old wording stops counting (ticket 93).
