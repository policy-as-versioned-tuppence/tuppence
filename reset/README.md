...
|---|---|---|
| **reach** | `authorizationpolicy.yaml` | Istio `AuthorizationPolicy` ALLOW, `source.principals: acme.internal/posture/5.0.0/*` (scheme-less — Istio prepends `spiffe://` itself) |
| **secret** | `openbao-role.yaml` | OpenBao jwt role, `bound_claims` glob `sub: spiffe://…/posture/5.0.0/*` (the JWT `sub` claim IS the full URI) |
...
```mermaid
flowchart LR
  subgraph current["teller-current — claims 5.0.0"]
    C["SVID posture/5.0.0/…"]
  end
...
The prefix is `5.0.0`. Every institution accepts it, and platform retires
4.0.0 (Phase B, 2026-09-24). Bump the bar (or retire 5.0.0)
